...
pub fn extract_shareable_pattern(
    ...
    debug!("Extracting shareable pattern from interaction {}", interaction.id);
    ...
    if config.pii_detection_enabled {
        let pii_detected = detect_pii(&insight);
    ...
    let sensitivity = assess_sensitivity(&generalized, &config.blocked_categories);
    if sensitivity.is_blocked {
        debug!("Pattern blocked due to category: {:?}", sensitivity.categories);
        return Ok(None);
    }
    
    if sensitivity.score > config.human_review_threshold {
        debug!("Pattern requires human review (sensitivity: {})", sensitivity.score);
...
fn infer_domain(query: &str) -> String {
    ...
    if query_lower.contains("write") || query_lower.contains("draft") || query_lower.contains("email") {
...
fn generate_description(trigger: &str, domain: &str) -> String {
    format!("Pattern for {} tasks: {}", domain, &trigger[..trigger.len().min(50)])
}
...
fn remove_pii(insight: &ExtractedInsight) -> ExtractedInsight {
    ...
    for pii in pii_matches.iter().rev() {
        ...
        result.template.replace_range(pii.start..pii.end, replacement);
    }
...
fn assess_sensitivity(text: &str, blocked_categories: &[String]) -> SensitivityAssessment {
    ...
    if text_lower.contains("bank") || text_lower.contains("account") || text_lower.contains("credit") {
        categories.push("financial".to_string());
        score += 0.4;
    }
    
    // Check for medical content
    if text_lower.contains("diagnosis") || text_lower.contains("patient") || text_lower.contains("medication") {
        categories.push("medical".to_string());
        score += 0.4;
    }
    
    // Check for legal content
    if text_lower.contains("lawsuit") || text_lower.contains("defendant") || text_lower.contains("plaintiff") {
...
fn compute_utility(privatized: &ExtractedInsight, original: &ExtractedInsight) -> f64 {
    ...
    let orig_words: std::collections::HashSet<_> = original.template
        .split_whitespace()
        .collect();
    let priv_words: std::collections::HashSet<_> = privatized.template
        .split_whitespace()
        .collect();
