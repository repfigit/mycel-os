...
impl Pattern {
    ...
    pub async fn apply(&self, context: &Context) -> Result<PatternResult> {
        match &self.solution {
            PatternSolution::PromptTemplate { template, variables } => {
            ...
            PatternSolution::Workflow { steps } => {
                Ok(PatternResult::Workflow(steps.clone()))
            }
            PatternSolution::ModelAdapter { base_model, adapter_cid, .. } => {
    ...
    pub fn solution_summary(&self) -> String {
        match &self.solution {
            PatternSolution::PromptTemplate { template, .. } => {
                format!("Prompt template: {}...", &template[..template.len().min(100)])
            }
...
pub enum PatternResult {
    ...
    Adapter { base_model: String, adapter_cid: String },
}
...
impl PatternStore {
    ...
    pub fn record_usage(&mut self, pattern_id: &PatternId) {
        let stats = self.usage_stats.entry(pattern_id.clone()).or_insert(UsageStats::default());
        stats.usage_count += 1;
