...
import type { AccessKeyWithPublicKey, BlockId, BlockReference, ChunkId, FinalityReference, AccessKeyView as InternalAccessKeyView, AccountView as InternalAccountView, ContractCodeView as InternalContractCodeView, LightClientProofRequest, NextLightClientBlockRequest, SerializedReturnValue, TxExecutionStatus } from '../types/index.js';
type Prettify<T> = {
    [K in keyof T]: T[K];
} & {};
...
export type ViewGlobalContractCodeArgs = Prettify<{
    identifier: {
        codeHash: CryptoHash | Uint8Array;
    } | {
        accountId: string;
    };
} & BlockQuery>;
...
export type ViewValidatorsArgs = {
    blockId: BlockId;
} | {
    epochId: string;
};
...
export interface Provider {
    ...
    accessKeyChanges(accountIdArray: string[], BlockQuery: BlockId | BlockReference): Promise<RpcStateChangesInBlockResponse>;
    singleAccessKeyChanges(accessKeyArray: AccessKeyWithPublicKey[], BlockQuery: BlockId | BlockReference): Promise<RpcStateChangesInBlockResponse>;
    accountChanges(accountIdArray: string[], BlockQuery: BlockId | BlockReference): Promise<RpcStateChangesInBlockResponse>;
    contractStateChanges(accountIdArray: string[], BlockQuery: BlockId | BlockReference, keyPrefix: string): Promise<RpcStateChangesInBlockResponse>;
    contractCodeChanges(accountIdArray: string[], BlockQuery: BlockId | BlockReference): Promise<RpcStateChangesInBlockResponse>;
